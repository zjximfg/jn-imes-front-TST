...
const OutWarehouseRebarForm: FC<TableFormProps> = ({value, onChange}) => {
  ...
  const handleFinish = (values: RebarMemberOutDataType[], sumOutQuantity: number, sumOutTheoreticalWeight: number) => {
  ...
  const columns = [
        ...
        if (record.editable && record.batchNumberList.length > 0) {
          return (
            <Select
              ...
              setBatchNumber(value as string);
            }}>
    ...
    },
    {
      ...
      render: (text: string, record: TableFormDateType) => {
        console.log("单位", record.editable)
        if (record.editable) {
          return (
            <Input
              value={text}
  ...
  return (
    <>
      ...
      <OutWarehouseRebarMember visible={visible} currentTableForm={currentTableForm} batchNumber={batchNumber} rebarMemberOutEdited={selectedRebarMemberOutList} onCancel={handleCancel} onFinish={handleFinish}/>
    </>
  );
};
