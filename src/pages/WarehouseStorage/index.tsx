...
const WarehouseStorage: React.FC<{}> = () => {
  ...
  const [selectedWarehouseStorage, setSelectedWarehouseStorage] = useState<WarehouseStorageDataType[]>([]);
  ...
  const handleSelectedChange = (selectedRowKeys: Key[], selectedRows: WarehouseStorageDataType[]) => {
    ...
    setSelectedWarehouseStorage(selectedRows);
  };
  ...
  const handleOutWarehouseFinish = (values: Partial<OutWarehouseDataType>) => {
    // TODO
    // 新建数据
  };
  ...
  return (
    <PageContainer>
      ...
      <OutWarehouseForm visible={outWarehouseVisible} warehouseStorageList={selectedWarehouseStorage} onCancel={handleCancel} onFinish={handleOutWarehouseFinish}/>
    </PageContainer>
  )
};
